...
class RiskScoringEngine:
    ...
    @classmethod
    def calculate_final_risk(cls, video_risk: float, audio_risk: float, evaluation_risk: float) -> float:
        """Calculate final combined risk score using weighted average."""
        video_weight, audio_weight, eval_weight = cls.get_pipeline_weights()
        final_risk = video_weight * video_risk + audio_weight * audio_risk + eval_weight * evaluation_risk
    ...
    def classify(cls, video: dict, audio: dict, evaluation: dict) -> str:
        ...
        bg_voice = audio.get("background_voices", {}).get("background_voices_detected", False)
        conv = audio.get("suspicious_conversation", {}).get("suspicious_pattern_detected", False)
        ...
        if evaluation.get("answer_quality_score", {}).get("overall_quality_score", 100) < 40:
            return "MEDIUM"
